# NutriBot - Llama2 7B Installation Script for Windows
# This script installs Ollama and sets up Llama2 7B for NutriBot

Write-Host "🤖 NutriBot - Instalación de Llama2 7B" -ForegroundColor Cyan
...
if ($totalRAM -lt 8) {
    Write-Host "❌ ADVERTENCIA: Se recomienda al menos 8GB de RAM para Llama2 7B" -ForegroundColor Red
...
if ($freeSpace -lt 8) {
    Write-Host "❌ ADVERTENCIA: Se necesitan al menos 8GB de espacio libre" -ForegroundColor Red
...
Write-Host "📥 Descargando modelo Llama2 7B..." -ForegroundColor Yellow
Write-Host "⚠️  Este modelo tiene aproximadamente 4GB y puede tomar 10-20 minutos..." -ForegroundColor Yellow
Write-Host "⚠️  Compatible con sistemas con 8-16GB de RAM..." -ForegroundColor Yellow

$continue = Read-Host "¿Deseas continuar con la descarga? (s/n)"
if ($continue -ne "s") {
    Write-Host "Puedes descargar el modelo manualmente más tarde con: ollama pull llama2:7b" -ForegroundColor Yellow
} else {
    try {
        Write-Host "🚀 Iniciando descarga de Llama2 7B..." -ForegroundColor Green
        ollama pull llama2:7b
        Write-Host "✅ Modelo Llama2 7B descargado correctamente" -ForegroundColor Green
    } catch {
        Write-Host "❌ Error al descargar el modelo. Puedes descargarlo manualmente con: ollama pull llama2:7b" -ForegroundColor Red
    }
}
...
Write-Host "🧪 Probando el modelo Llama2 7B..." -ForegroundColor Yellow
try {
    ...
    if ($response.response) {
        Write-Host "✅ Modelo Llama2 7B funcionando correctamente" -ForegroundColor Green
...
Write-Host ""
Write-Host "🔧 Configuración para NutriBot con Llama2 7B:" -ForegroundColor Cyan
Write-Host "=============================================" -ForegroundColor Cyan
Write-Host "1. Asegúrate de que USE_LOCAL_LLM=true en tu archivo .env" -ForegroundColor White
Write-Host "2. El modelo por defecto es llama2:7b" -ForegroundColor White
Write-Host "3. Ollama se ejecuta en http://localhost:11434" -ForegroundColor White
Write-Host "4. Compatible con sistemas con 8-16GB de RAM" -ForegroundColor White
Write-Host ""
Write-Host "📝 Comandos útiles:" -ForegroundColor Cyan
Write-Host "  - ollama list                    # Listar modelos" -ForegroundColor Gray
Write-Host "  - ollama pull llama2:7b         # Descargar modelo" -ForegroundColor Gray
Write-Host "  - ollama run llama2:7b          # Ejecutar modelo interactivo" -ForegroundColor Gray
Write-Host "  - ollama serve                  # Iniciar servicio" -ForegroundColor Gray
Write-Host ""
Write-Host "⚡ Optimizaciones para Llama2 7B:" -ForegroundColor Cyan
...
Write-Host "  - Buen rendimiento en consultas nutricionales" -ForegroundColor Gray
Write-Host ""
Write-Host "🎉 ¡Instalación completada! Ahora puedes ejecutar NutriBot con Llama2 7B." -ForegroundColor Green
